        ...
        {todos?.map((item) => (
          <div className={styles.todosBox}>
            ...
            <ul>
              {item.content?.map((it) => (
                <>
                  <li className={styles.todolist} draggable>
                    <span variant="outlined" onClick={handleClickOpen}>
                      {it.content}{" "}
                    </span>
                    <span
                      onClick={() => handleCloseList(item.id, it.id)}
                  ...
                  <Dialog
                    style={{ minWidth: "900px", margin: "10px auto" }}
                    open={open}
                    onClose={handleClose1}
                    aria-labelledby="alert-dialog-title"
                    aria-describedby="alert-dialog-description"
                  >
                    <Description data={item} cardName={item.header} />
                  </Dialog>
                </>
              ))}
            </ul>
